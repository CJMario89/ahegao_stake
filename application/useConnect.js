...
const useConnect = () => {
  ...
  const connect = async () => {
    if (window?.ethereum) {
      try {
        ...
        console.log(signer);
        const response = await fetch("http://13.115.250.186/ /getMessage", {
          method: "POST",
        ...
        const response_ = await fetch("http://13.115.250.186/ /sign-in", {
          method: "POST",
  ...
  //         });
  //         //check jwt
  //         const response = await fetch("http://13.115.250.186/ /getJWTAddress", {
  //           method: "GET",
  //           credentials: "include",
  //           headers: {
  //             Accept: "application/json",
